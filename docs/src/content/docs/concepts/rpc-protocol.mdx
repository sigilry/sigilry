...
- `accountsChanged`
- `txChanged`
- `statusChanged` — CIP-103 §4.2.2 provider status transitions; disconnects route through here per §4.2.2:216
- `connected` — CIP-103 login-flow completion event (not one of the §4.2.2 three); identical payload to `statusChanged`
